...
const NAV = [
  ...
  { page: 'users',   label: 'Users', icon: <svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg> },
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated' && u?.role !== 'admin') router.push('/login');
  ...
  return (
    <div className="app">
      <Sidebar items={NAV} activePage={page} onNavigate={setPage} username={user?.username || ''} role="Admin" />
      <div className="main">
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: page === 'pos' ? 24 : 24, display: 'flex', flexDirection: 'column' }}>
          {page === 'reports'   && <ReportsView isAdmin={true} />}
          {page === 'menu'      && <MenuManagement />}
          {page === 'inventory' && <InventoryView />}
          {page === 'users'     && <UserManagement />}
          {page === 'pos'       && <POSView username={user?.username || ''} />}
        </div>
      </div>
      <ToastContainer />
    </div>
